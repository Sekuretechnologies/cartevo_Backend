  ...
  describe('generateToken', () => {
    it('should generate token for valid credentials', async () => {
      ...
      const mockBusiness = {
        ...
        clientKey: hashedKey,
        cardPrice: new Decimal(5.00),
        cardFundRate: new Decimal(1.02),
        isActive: true,
        createdAt: new Date(),
        updatedAt: new Date(),
      };

      jest.spyOn(prismaService.company, 'findUnique').mockResolvedValue(mockBusiness);
    ...
    it('should throw UnauthorizedException for invalid client_id', async () => {
      ...
      await expect(service.generateToken(authDto)).rejects.toThrow(UnauthorizedException);
    });
